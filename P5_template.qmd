...
For this project it was a lot of work on cleaning up the data, and there are a lot of ways to do that. A lot of the questions that go along with that are quite subject (including what to do with missing values). So it's not very surprising that the article that used the same data didn't get the exact same numbers from the same data that I did. But the general insights were the same as from the article, and I made a random forest classifier that predicts with 91% recall who of the survey answerers make over $50k a year. 
...
df = pd.get_dummies(df, columns=['who_shot_first','gender','location_census_region'])
...
```
Here I one-hot encoded the 'who_shot_first', 'gender', and 'location_census_region' columns. Those were all the columns left that were categorical (and gave them mappings of numerical values didn't really make sense).

```{python}
df_ml = df.dropna(subset='household_income')
df_ml['y'] = (df_ml.household_income >= 50).astype(int)

# Separate the X and y features
X = df_ml.drop(columns=['respondent_id', 'household_income', 'y'])
y = df_ml.y
```
Here I created and separated the X and y dataframes. 
...
I did not get the exact same results as the graphs from the article had. But I could see how maybe the way I cleaned my data compared to what they might have done could have changed the results slightly. Overall the general ideas are the same, if the numbers aren't exactly the same. 

```{python}
seen_movies = ['seen_episode_i', 'seen_episode_ii', 'seen_episode_iii', 'seen_episode_iv', 'seen_episode_v', 'seen_episode_vi']

percentage_seen = pd.DataFrame({
    'movie': seen_movies,
    'percentage_seen': (df[seen_movies].mean() * 100).round().astype(int)
})
movie_titles = {
...
percentage_seen['movie'] = percentage_seen['movie'].map(movie_titles)
percentage_seen['movie'] = pd.Categorical(
    percentage_seen['movie'],
    categories=list(movie_titles.values())[::-1],
    ordered=True
  )
(
  ggplot(percentage_seen, aes(x="movie", y="percentage_seen"))
  + geom_bar(stat='identity', color='#f0f0f0')
  + coord_flip()
  + labs(
  ...
  + geom_text(x=5, y=87, label="81%", family="Consolas")
  + geom_text(x=4, y=72, label="68", family="Consolas")
  + geom_text(x=3, y=70, label="66", family="Consolas")
  + geom_text(x=2, y=77, label="73", family="Consolas")
  + geom_text(x=1, y=95, label="91", family="Consolas")
  + geom_text(x=0, y=92, label="88", family="Consolas")
  + theme(
    plot_title=element_text(size=20, hjust=-0.55, family="Consolas", face='bold'),
    plot_subtitle=element_text(hjust=-0.55, family="Consolas"),
    axis_line=element_blank(),
    axis_ticks=element_blank(),
    panel_grid=element_blank(),
    axis_text_y=element_blank(),
    axis_text_x=element_text(margin=[0,10], family="Consolas"),
    plot_background=element_rect(color="black", fill="#f0f0f0", size=0)
  )
)
```

```{python}
not_na = df.dropna(subset=['rank_episode_i', 'rank_episode_ii', 'rank_episode_iii', 'rank_episode_iv', 'rank_episode_v', 'rank_episode_vi'], how='any')
df_ranks = not_na[['rank_episode_i', 'rank_episode_ii', 'rank_episode_iii', 'rank_episode_iv', 'rank_episode_v', 'rank_episode_vi']]
df_long = df_ranks.melt(var_name='Movie', value_name='Rank')
rank_1_counts = df_long[df_long['Rank'] == 1]['Movie'].value_counts()
total_rows = len(df_ranks)  # Total number of respondents (original rows in df)
rank_1_percentage = ((rank_1_counts / total_rows) * 100).round().astype(int)
result_df = rank_1_percentage.reset_index()
result_df.columns = ['movie', 'percentage_ranked_first']
...
result_df['movie'] = result_df['movie'].map(movie_titles)
result_df['movie'] = pd.Categorical(
    result_df['movie'],
    categories=list(movie_titles.values())[::-1],
    ordered=True
  )
(
  ggplot(result_df, aes(x="movie", y="percentage_ranked_first"))
  + geom_bar(stat='identity', color='#f0f0f0')
  + coord_flip()
  + labs(
    title="What's the best 'Star Wars' Movie?",
    subtitle="Of 833 respondents who have seen all six films",
    x='',
    y=''
  )
  + geom_text(x=5, y=17, label="15%", family="Consolas")
  + geom_text(x=4, y=5, label="4", family="Consolas")
  + geom_text(x=3, y=5, label="4", family="Consolas")
  + geom_text(x=2, y=26, label="24", family="Consolas")
  + geom_text(x=1, y=36, label="34", family="Consolas")
  + geom_text(x=0, y=20, label="18", family="Consolas")
  + theme(
    plot_title=element_text(size=20, hjust=-0.55, family="Consolas", face='bold'),
    plot_subtitle=element_text(hjust=-0.55, family="Consolas"),
    axis_line=element_blank(),
...
I created a Random Forest Classifier that predicts whether or not the person answering the survey makes over $50k a year based off their answers to the other questions. It answers with about a 64% accuracy, which isn't great. But it does have a fairly high recall, of about 91%. So it's really good at finding all of the people above $50k, it's just a little trigger-happy and is guessing that most people make above $50k. My guess as to why this might be is that the dataset was originally pretty unbalanced, and it's not great at balancing itself. 
But looking at the feature importance, it makes sense that education and age are the biggest identifiers for how much someone makes. 
...
forest = RandomForestClassifier(class_weight='balanced', n_estimators=500, bootstrap=False, random_state=42)
...
ggplot(data = influences, mapping = aes(x = 'feature', y = 'importance'))
  ...
  + theme(plot_title=element_text(size=15, hjust=-.3))
)
```
---
